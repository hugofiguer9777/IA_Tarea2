  ...
  try {
    if (posicion.direccion == "N") {
      ...
      if (bloquear(posicion)) {
        return followTrail(nex_position, calcularValor(nex_position));
      }
      return {
        ...posicion,
...
function mover(movimientos, turno, max, index) {
...
app.get("/", (req, res) => {
  ...
  if (estado) {
    mapToMatrix(estado);
    let posiciones = getMyposiciones(turno, tableroJuego);
    ...
    let move_to = `${mover(arbol, turno, -9999999, 0)}`;
